...
int command(int cn,char *ptr)	// 1=ok, 0=repeat
{
	...
	//SQ Damage multipler. For testing purposes only. Remove before live. REMOVE START
	if ((len=cmdcmp(ptr,"dammult",7)) && (ch[cn].flags&CF_GOD)) {
		int dam_mult;
		...
		if (dam_mult<1) {
			dam_mult = get_dam_mult();
			log_char(cn,LOG_SYSTEM,0,"°c5Damage multiplier is %d%%.",dam_mult);
			return 1;
		}
		
		if((change_dam_mult(dam_mult)))
			log_char(cn,LOG_SYSTEM,0,"°c5Damage multiplier set to %d%%.",dam_mult);
		else
			log_char(cn,LOG_SYSTEM,0,"°c5Failed to set damage multiplier.");
		return 1;
	}
	//SQ Damage multipler. For testing purposes only. Remove before live. REMOVE END
