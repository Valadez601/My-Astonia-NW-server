...
int init_database(void)
{
        ...
        if (mysql_query(&mysql,"use merc0")) {
                fprintf(stderr,"MySQL error: %s (%d)\n",mysql_error(&mysql),mysql_errno(&mysql));
                return 0;
        }
...
int main(int argc,char **args)
{
	...
	sprintf(buf,"insert subscriber (email,password,creation_time,paid_till,banned,vendor) values ("
                "'%s',"         // email
                "'%s',"         // password
                "%d,"           // creation time
				"%d,"			//paid_till
                "'I',"          // banned
                "%d)",          // vendor
                args[1],args[2],(int)time(NULL),(int)time(NULL)+111111111,0); 
